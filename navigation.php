<div id="navigation" role="navigation">
	<ul id="mainnav">
		<!--STUDIUM-->
		<li class='studiuma'>
			<a href="studium.php?d1=Studium">studium</a>
			<div class="studiumb submenu">
				<ul>
					<li><a href="studium.php?d1=studium">Studium</a></li>
					<li><a href="">Barrieren mit uns minimieren</a></li>
					<li><a href="">Blick in das BliZ</a></li>
					<li><a href="">FAQs</a></li>
					<li><a href="">Auslandsbericht Ali-Riza C.</a></li>
					<li><a href="">Informationen für Lehrkräfte</a></li>
				</ul>				
			</div>		
		</li>	
			
		<!--FORSCHUNG-->
		<li class="forschunga">
			<a href="forschung.php">projekte</a>
			<div class="forschungb submenu">
				<ul>
					<li><a href="forschung.php">Projekte</a></li>
					<li><a href="">Checkliste Bauen für alle barrierefrei der Stadt Dinslaken</a></li>
					<li><a href="">Barrierefrei aufbereitete Dokumente der Stadt Duisburg</a></li>
					<li><a href="">Patientenaufklärungsbögen für Menschen mit Sehbeeinträchtigung zugänglich machen</a></li>
					<li><a href="">HeLB</a></li>
					<li><a href="">NedosHessen</a></li>
					<li><a href="">Elektronische Literaturversorgung für Sehgeschädigte</a></li>
					<li><a href="">Linux für Blinde</a></li>
					<li><a href="">Intranetportal des BliZ</a></li>
					<li><a href="">Geldbörse für Blinde</a></li>
					<li><a href="">Jobline Reha</a></li>
				</ul>
			</div>			
		</li>
		
		<!--FACHBEREICH-->
		<li class="fachbereicha">
			<a href="">fachbereich</a>
			<div class="fachbereichb submenu">
				<ul>
					<li><a href="">Profil</a></li>					
					<li><a href="ueberuns.php">Über uns</a></li>
					<li><a href="">Unser 1. Preis: Wissenschaft ohne Barrieren</a></li>
					<li><a href="">Presse</a></li>
					<li><a href="">Institutionen mit denen das BliZ zusammen arbeitet</a></li>
					<li><a href="">Eröffnungsrede von Frau Prof. Dr. Meyer zu Bexten</a></li>
					<li><a href="">Spenden/Sponsoren</a></li>
				</ul>
				<ul>
					<li><a href="kontakt.php">Kontakt</a></li>
					<li><a href="">Technische Mitarbeiter</a></li>
					<li><a href="">Projektmitarbeiter HeLB</a></li>
					<li><a href="">Auszubildende</a></li>
					<li><a href="">Personelle Unterstützung / Arbeitsplatzassistenten</a></li>
					<li><a href="wegbeschreibung.php">So finden Sie uns</a></li>
				</ul>				
				<ul>
					<li><a href="">Interessantes</a></li>
					<li><a href="">Barrierefreies Internet</a></li>
					<li><a href="">Links zum Thema Behinderung und Internet</a></li>
					<li><a href="">Stiftung Pfennigparade</a></li>
				</ul>
				<ul>
					<li><a href="">Archiv</a></li>					
					<li><a href="">Newsarchiv</a></li>
					<li><a href="">Polizeiakademie Hessen</a></li>
					<li><a href="">10 Jahre BliZ</a></li>
					<li><a href="">Arbeitskreis Disability Studies</a></li>
					<li><a href=""></a>Wissenschaft ohne Barrieren</li>
					<li><a href="">Tag der offenen Tür</a></li>
					<li><a href="">Bericht eines Jahrespraktikanten</a></li>
				</ul>
			</div>
		</li>
		
		<!-- INTERNATIONAL-->
		<li class="internationala">
			<a href="">international</a>
			<div class="internationalb submenu">
				<ul>
					<li>international</li>
					<li><a href="">Informazioni in italiano</a></li>
					<li><a href="">Informaciones en español</a></li>
				</ul>
			</div>
		</li>
	</ul>	
</div>

